...
export default function TestConfig() {
  ...
  if (errorMsg || !qs) {
    return (
      <div className="max-w-md mx-auto my-12 p-8 bg-white border border-slate-200 rounded-2xl text-center shadow-md">
        <div className="w-12 h-12 bg-rose-100 text-rose-600 rounded-full flex items-center justify-center mx-auto mb-3">
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
          </svg>
        </div>
        <h2 className="text-xl font-bold text-slate-900 mb-1">Set Not Found</h2>
        <p className="text-slate-500 text-sm mb-6">{errorMsg || "Unable to load the requested practice set."}</p>
        <Link to="/" className="inline-block bg-slate-900 text-white font-semibold text-sm px-5 py-2.5 rounded-xl hover:bg-slate-800 transition-colors">
          Return to Dashboard
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-lg mx-auto my-8 sm:my-16 px-4">
      <div className="bg-white border border-slate-200/80 rounded-3xl shadow-xl p-6 sm:p-8 animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-100">
          ...
          <div>
            <h1 className="text-xl font-extrabold text-slate-900">Configure Practice Test</h1>
            <p className="text-xs font-semibold text-slate-400">Set your timer preferences before starting</p>
          </div>
        </div>

        {/* Question Set Info Card */}
        <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200/80 mb-6">
          <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Selected Question Bank</span>
          <h2 className="text-lg font-bold text-slate-900 mt-0.5">{qs.name}</h2>
          <div className="flex items-center gap-3 mt-2 text-xs font-semibold text-slate-600">
            <span className="bg-blue-100/80 text-blue-700 px-2.5 py-0.5 rounded-md">
